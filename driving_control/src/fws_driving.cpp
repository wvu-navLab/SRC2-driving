...
#include "driving_control/fws_driving.hpp"

FourWheelSteeringDriving::FourWheelSteeringDriving(ros::NodeHandle &nh)
    : nh_(nh), command_struct_twist_()
{
  ...
  // Server
  serverEnableDriving = nh_.advertiseService("driving/enable", &FourWheelSteeringDriving::EnableDriving, this);
}

void FourWheelSteeringDriving::Starting(const ros::Time &time)
{
  ROS_INFO_STREAM("[" << robot_name_ << "] "
                      << "Starting 4WS Driving.");
  Brake();
}
...
bool FourWheelSteeringDriving::EnableDriving(driving_control::EnableDriving::Request &req, driving_control::EnableDriving::Response &res)
{
  enable_twist_cmd_ = req.enable;
  if (!req.enable)
  {
    // Set wheels velocities:
    motion_control::WheelGroup w;
    w.w1 = 0.0; //fr_wheel_joint
    w.w2 = 0.0;  //br_wheel_joint
    w.w3 = 0.0;  //fl_wheel_joint
    w.w4 = 0.0;   //bl_wheel_joint
...
void FourWheelSteeringDriving::UpdateCommand(const ros::Time &time, const ros::Duration &period)
{
  if (active_cmd_vel_ == true)
  {
    ...
    else
    {
      ...
      if (enable_twist_cmd_ == true)
      {
        ...
        else // Double Ackermann Driving
        {
          ...
          if (fabs(curr_cmd_twist.lin_x) > 0.001)
          {
            ...
            vel_left_front = sign * std::hypot((curr_cmd_twist.lin_x - curr_cmd_twist.ang * steering_track / 2), (wheel_separation_length_ * curr_cmd_twist.ang / 2.0)) / wheel_radius_ - vel_steering_offset;
            vel_right_front = sign * std::hypot((curr_cmd_twist.lin_x + curr_cmd_twist.ang * steering_track / 2), (wheel_separation_length_ * curr_cmd_twist.ang / 2.0)) / wheel_radius_ + vel_steering_offset;
            vel_left_rear = sign * std::hypot((curr_cmd_twist.lin_x - curr_cmd_twist.ang * steering_track / 2), (wheel_separation_length_ * curr_cmd_twist.ang / 2.0)) / wheel_radius_ - vel_steering_offset;
            vel_right_rear = sign * std::hypot((curr_cmd_twist.lin_x + curr_cmd_twist.ang * steering_track / 2), (wheel_separation_length_ * curr_cmd_twist.ang / 2.0)) / wheel_radius_ + vel_steering_offset;
          }
      ...
      pubWheelVels.publish(w);
      // ROS_INFO_STREAM("[" << robot_name_ << "] 4WS. Wheel velocities commanded" << w);
      ...
      // ROS_INFO_STREAM("[" << robot_name_ << "] 4WS. Steering angles commanded" << s);
    }

    std_msgs::Int64 mode;
    mode.data = driving_mode_;
    pubDrivingMode.publish(mode);
    // ROS_INFO_STREAM("[" << robot_name_ << "] 4WS. Driving mode published" << mode);
  }
}
